...
struct cmd_spec cmd_table[] = {
    ...
    { "mcd-cache-mode",
      &main_mcd_cache_mode,
      "Set cache mode",
      "[-c[=CACHE MODE]] [-r[=REPLACEMENT MODE]]",
      " -c CACHE MODE                   0 = shared, 1 = private\n"
      " -r REPLACEMENT MODE             0 = slow,   1 = fast",
    },
    { "mcd-ssd-param",
      &main_mcd_ssd_param,
      "Set SSD Param",
      "[-r[=READ LATENCY NSEC]] [-w[=WRITE LATENCY NSEC]] [-s[=SSD SIZE PERCENT]]",
      " -r READ LATENCY in NSEC\n"
      " -w WRITE LATENCY in NSEC\n"
      " -s USAGE RATE in PERCENT (100 = same as cache size)",
    },
    { "network2-attach",
      &main_network2attach,
