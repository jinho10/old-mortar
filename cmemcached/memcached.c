...
//------------------- helper
#if 0
static void fdump(char* key, char* buf, int size)
{
    //int i;
    FILE *file;
...
fprintf(stderr, "--------- file size = %d \n", size);
    ...
    /*
    for(i=0; i<(size/4); i++) {
        fprintf(stderr, "0x%x ", val[i]);
    }
    fprintf(stderr, "\n");
    */
}

static int mfset(char *key, int key_len, char* val, int val_size)
{
    FILE *file;
    char filename[128];
    int i;
    ...
    if ( file == NULL ) {
        fprintf(stderr, "set file open failed...\n");
        return -1;
    }

    ret = fwrite(val, 1, val_size, file);
...
static int mfgetsize(char *key, int key_len)
{
    FILE *file;
    char filename[128];
    int i;
...
static int mfget(char *key, int key_len, char* val, int val_size)
{
    FILE *file;
    char filename[128];
    int i;
...
static void complete_incr_bin(conn *c) {
    ...
    if (settings.verbose > 1) {
        int i;
        fprintf(stderr, "complete_incr_bin incr ");
...
static void process_bin_get(conn *c) {
    ...
    //fprintf(stderr, "binary nkey ----------------- %ld \n", nkey);

    // TODO jinho get hypervisor buffer
    mcd_verbose(XENMCD_cache_check, MCDOPT_shared, nkey, key, 0, &val_size, 0);
    if ( val_size < 0 ) {
        if (settings.verbose > 1) 
            fprintf(stderr, "<%d bin GET failed to get size for key (%s) : %d\n", c->sfd, key, val_size);

        goto not_found;
    }
    
    val = (char*) malloc(val_size);
    mcd_verbose(XENMCD_cache_get, MCDOPT_shared, nkey, key, val_size, &ret_val, val);
    //fprintf(stderr, "binary ----------------- %d \n", ret_val);
    if ( ret_val < 0 ) {
        if (settings.verbose > 1) 
            fprintf(stderr, "<%d bin GET failed to get : %d \n", c->sfd, ret_val);
    ...
    /* the length has two unnecessary bytes ("\r\n") */
    keylen = 0;
    bodylen = sizeof(rsp->message.body) + (val_size - 2);

    //fprintf(stderr, "binary bodylen ----------------- %d \n", bodylen);
    ...
    /* Add the data minus the CRLF */
    add_iov(c, val, val_size - 2);
    conn_set_state(c, conn_mwrite);
    c->write_and_go = conn_new_cmd;

    /* Remember this command so we can garbage collect it later */
    //c->item = it; // does this make problem??

    c->hyperbuf = val;  /* should be freed after the get message transmitted */

    return;

not_found: // when data does not exist
    if (c->noreply) {
        conn_set_state(c, conn_new_cmd);
    } else {
        if (c->cmd == PROTOCOL_BINARY_CMD_GETK) {
...
static void process_bin_delete(conn *c) {
    ...
    // Assume success
    mcd_verbose(XENMCD_cache_remove, MCDOPT_shared, nkey, key, 0, &ret_val, 0);
    if ( ret_val < 0 ) {
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_KEY_ENOENT, 0); // not found
    } else {
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_KEY_EEXISTS, 0); // found
    }
#else
    item *it;
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    ...
    if ( c->protocol == binary_prot ) {
        ...
        val = ITEM_suffix(it);
        //ITEM_suffix(it), it->nsuffix + it->nbytes;
    }
...
/* unnecessary...
    mcd_verbose(XENMCD_cache_check, MCDOPT_shared, it->nkey, key, 0, &ret_val, 0);

    if ( ret_val > 0 ) {
        if (settings.verbose > 1) 
            fprintf(stderr, "<%d FOUND KEY: %s \n", c->sfd, key);

        // XXX jinho is this right?????
        //return NOT_STORED;
    }
*/

    // TODO jinho - hypervisor call
    //fdump(key, val, it->nbytes);

//fprintf(stderr, "store_item = key %s, val_size %d \n", key, it->nbytes);

    if ( c->protocol == binary_prot ) {
        mcd_verbose(XENMCD_cache_put, MCDOPT_shared, it->nkey, key, it->nbytes, &ret_val, val);
    } else {
        mcd_verbose(XENMCD_cache_put, MCDOPT_shared, it->nkey, key, (it->nsuffix + it->nbytes), &ret_val, val);
    }
    //ret_val = mfset(key, it->nkey, val, it->nbytes);
    //ret_val = mfset(key, it->nkey, val, (it->nsuffix + it->nbytes));

    if ( ret_val < 0 ) {
        if (settings.verbose > 1) 
            fprintf(stderr, "<%d store_item PUT failed : %d \n", c->sfd, ret_val);

        return NOT_STORED;
    }

//fprintf(stderr, "set --------- %s -------- size = %d \n", key, it->nbytes);
...
//---- jinho
            mcd_verbose(XENMCD_cache_check, MCDOPT_shared, nkey, key, 0, &val_size, 0);
//fprintf(stderr, "get_command = key %s, val_size %d \n", key, val_size);
            //val_size = mfgetsize(key, nkey);
            if ( val_size < 0 ) {
                if (settings.verbose > 1) 
                    fprintf(stderr, "<%d ascii GET failed to get size for key (%s) : %d \n", c->sfd, key, val_size);
            ...
            mcd_verbose(XENMCD_cache_get, MCDOPT_shared, nkey, key, val_size, &ret_val, val);
            //ret_val = mfget(key, nkey, val, val_size);

            //fprintf(stderr, "ascii ----------------- %d \n", ret_val);
            if ( ret_val < 0 ) {
                if (settings.verbose > 1) 
                    fprintf(stderr, "<%d ascii GET failed to get : %d \n", c->sfd, ret_val);
                //out_string(c, "NOT_FOUND");
                free(val);
...
//fdump(key, val, val_size);
//fprintf(stderr, "get -------- %s -------- size = %d \n", key, val_size);

            // will be freed after transmission
            c->hyperbuf = val;
...
//----
            //it = item_get(key, nkey);
            if (settings.detail_enabled) {
                stats_prefix_record_get(key, nkey, !NULL);
            }
            ...
            if (return_cas)
            {
                // Faban does not come here
                fprintf(stderr, "<%d should not be here... \n", c->sfd);
            ...
            else
            {
                ...
                char suffix[128];
                int suffix_len;
                sprintf(suffix, " 0 %d\r\n", (val_size)); // XXX am I sure with -2 ??? exclude \r\n
                suffix_len = strlen(suffix);

                //real_len = strlen(length);
                //fdump(key, nkey);
                //fdump(suffix, suffix_len);
                //fdump(val, val_size);
             ...
             */
                ...
                // Response packet: VALUE <key> <flags> <bytes> [<cas unique>]\r\n
                //                  <data block>\r\n
                if (add_iov(c, "VALUE ", 6) != 0 ||
                    add_iov(c, key, nkey) != 0 || // does not end with \r\n
                    add_iov(c, suffix, suffix_len) != 0 || // end with \r\n
                    add_iov(c, val, val_size) != 0) // data block + \r\n
                    {
                        // error
                        if (settings.verbose > 1)
                            fprintf(stderr, "<%d ascii Assemble rsp failed %s\n", c->sfd, key);
                        //item_remove(it);
                ...
                // XXX for PHP - somehow PHP memcache.so does not work with above code
                if (add_iov(c, "VALUE ", 6) != 0 ||
                    add_iov(c, key, nkey) != 0 || // does not end with \r\n
                    add_iov(c, val, val_size) != 0) // suffix + data block + \r\n
                    {
                        // error
                        if (settings.verbose > 1)
                            fprintf(stderr, "<%d ascii Assemble rsp failed %s\n", c->sfd, key);
...
//ITEM_suffix(it), it->nsuffix + it->nbytes;
            ...
            if (settings.verbose > 1)
                fprintf(stderr, ">%d ascii sending key %s\n", c->sfd, key);

            key_token++;
        }

        // Let's go out to send error (e.g., NOT_FOUND) to client
        if ( escape ) break;
    ...
    */
    ...
    // in cmemcached, we assume that we do not have out-of-memory
    //if (key_token->value != NULL || add_iov(c, "END\r\n", 5) != 0

    if ( escape == 1 ) {
        if (settings.verbose > 1)
            fprintf(stderr, ">%d NOT_FOUND.. this is log..\n", c->sfd);
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    // jinho: will be removed after storing data into hypervisor
...
//    char temp[INCR_MAX_STORAGE_LEN];
    ...
    if ( val_size < 0 ) {
        ...
        // fake for now... because I do not have incr, desc functions... not a big deal
        // just do not make applications wait return value
        out_string(c, "2"); 
    }
...
// TODO jinho follow-up for this function to manage all the operations
static void process_command(conn *c, char *command) {
...
#if 0
void printf_header(protocol_binary_request_header *h)
{
    struct request *r = &h->request;
    printf("magic    : %d\n", h->request.magic;
...
//fprintf(stderr, "[%d] try_read_network avail(%d), c->rbytes (%d)\n", c->sfd, avail, c->rbytes);
        ...
        if (res == 0) {
//fprintf(stderr, "[%d] try_read_network res == 0\n", c->sfd);
            //conn_set_state(c, conn_waiting); // TODO this can cause infinite loop
            return READ_ERROR;
        }
        if (res == -1) {
//fprintf(stderr, "[%d] try_read_network res == -1\n", c->sfd);
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
        ...
        //fdump((char*)m->msg_iov->iov_base, (int)m->msg_iov->iov_len);

        res = sendmsg(c->sfd, m, 0);

        if (settings.verbose > 0) {
            //int len = (m->msg_iov->iov_len > 30) ? 30:m->msg_iov->iov_len; 
            fprintf(stderr, "%d: res (%d), m->msg_iovlen (%d), m->msg_iov->iov_len (%d) \n", 
                            c->sfd,
                            (int)res, 
                            (int)m->msg_iovlen, 
                            (int)m->msg_iov->iov_len);

            //fdump((char*)m->msg_iov->iov_base, len);
            //fdump((char*)m->msg_iov->iov_base, m->msg_iov->iov_len);
        }
    ...
    while (!stop) {
        ...
        case conn_nread:
            ...
            if (res == 0) { /* end of stream */
                conn_set_state(c, conn_closing);
                //conn_set_state(c, conn_push_hype);
                break;
            }
            if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
                if (!update_event(c, EV_READ | EV_PERSIST)) {
        ...
        case conn_mwrite:
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            ...
            case TRANSMIT_COMPLETE:
                ...
                if (settings.verbose > 0)
                    fprintf(stderr, "%d: TRANSMIT_COMPLETE\n", c->sfd);
            ...
            case TRANSMIT_INCOMPLETE:
                if (settings.verbose > 0)
                    fprintf(stderr, "%d: TRANSMIT_INCOMPLETE\n", c->sfd);

                break;
            case TRANSMIT_HARD_ERROR:
                if (settings.verbose > 0)
                    fprintf(stderr, "%d: TRANSMIT_HARD_ERROR\n", c->sfd);
                break;                   /* Continue in state machine. */

            case TRANSMIT_SOFT_ERROR:
                if (settings.verbose > 0)
                    fprintf(stderr, "%d: TRANSMIT_SOFT_ERROR\n", c->sfd);
